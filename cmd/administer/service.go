...
func CommandLine(provider configuration.Provider, name string) (patterns.CommandLine, error) {
	if name == services.ATTEMPT_TRIGGER_CLI {
		return ioc.AttemptTriggerCli(provider)
	}

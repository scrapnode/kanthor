...
func New(
	...
	logger = logger.With("service", "attempt.endeavor.executor")
	return &executor{
		conf:       conf,
		logger:     logger,
		...
		healthcheck: background.NewServer(
			healthcheck.DefaultConfig("attempt.endeavor.executor"),
...
func (service *executor) Run(ctx context.Context) error {
	topic := constants.TopicEndeavor
	if err := service.subscriber.Sub(ctx, topic, RegisterConsumer(service)); err != nil {
		return err
	}

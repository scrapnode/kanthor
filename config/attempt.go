...
func (conf *AttemptTriggerExecutor) Validate() error {
	return validator.Validate(
		validator.DefaultConfig,
		validator.NumberGreaterThanOrEqual("config.attempt.trigger.executor.timeout", int(conf.Timeout), 1000),
		validator.NumberGreaterThan("config.attempt.trigger.executor.size", conf.Size, 0),
		validator.NumberGreaterThanOrEqual("config.attempt.trigger.executor.attempt_delay", conf.AttemptDelay, 60000),
	)
}

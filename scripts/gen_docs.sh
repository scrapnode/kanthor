...
CHECKSUM_OLD=$(cat $PORTAL_FILE_CHECKSUM || true)
if [ "$CHECKSUM_NEW" != "$CHECKSUM_OLD" ];
then
  echo "generating portal docs ...";
  rm -rf "$PORTAL_FOLDER/docs"
  swag init -q --instanceName Portal -d $PORTAL_FOLDER -o $PORTAL_FOLDER/docs -g swagger.go --parseDependency --parseInternal;
...
CHECKSUM_OLD=$(cat $SDK_FILE_CHECKSUM ||  true)
if [ "$CHECKSUM_NEW" != "$CHECKSUM_OLD" ];
then
  echo "generating sdk docs ...";
  rm -rf "$SDK_FOLDER/docs"
